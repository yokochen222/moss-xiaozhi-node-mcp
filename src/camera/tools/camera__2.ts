...
const camera = new onvif.Cam({
  ...
  console.time('getSnapshotUri')
  camera.getSnapshotUri((error: any, snapshotUri: {
    uri: string
  }) => {
    console.timeEnd('getSnapshotUri')
    if (error) {
      ...
      console.time('fetchWithDigestAuth')
      // 使用 Digest 认证获取快照
      fetchWithDigestAuth(url, username, password)
        ...
        .then(async (data: any) => {
          console.timeEnd('fetchWithDigestAuth')
          if (data) {
  ...
  // moveCamera('down', 1)
  // camera.getStatus({}, function(err: Error | null, status: any) {
  //   if (err) {
